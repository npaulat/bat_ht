...
4. Get TE counts with [**te-counts.sh**](https://github.com/npaulat/bat_ht/blob/main/te-counts.sh)
5. Get third column from these TE count files
    * for i in \*\_HT; do awk -F ' ' '{print $3}' ${i} > ${i}2; done
6. Paste together these HT2 files in order of the species phylogeny using [**paste_final_table.sh**](https://github.com/npaulat/bat_ht/blob/main/final_paste_sp_table.sh); copy final_table to working directory as final_sp_TE_table
7. Run [**final_generate_heatmap_tables.py**](https://github.com/npaulat/bat_ht/blob/main/final_generate_heatmap_tables.py) in order to reformat and filter TE data for DNA/RC elements with limited distributions involving bats (final_sp_TE_heatmap_min100_DNA_RC_only.csv)
8. Use final_sp_TE_heatmap_min100_DNA_RC_only.csv as input for [**final_generate_blast_90_sh.py**](https://github.com/npaulat/bat_ht/blob/main/final_generate_blast_90.py), which will generate job submission scripts
...
11. Run [**final_make_blast_summary.py**](https://github.com/npaulat/bat_ht/blob/main/final_make_blast_summary.py) to create a summary table of BLAST results across all species (potential_ht_3_90_sp_hits_summary.csv)
...
13. Use ht_te_list as input file in [**blast_array.sh**](https://github.com/npaulat/bat_ht/blob/main/blast_array.sh) (make sure to change -a 1-N; N=length of ht_te_list), submit blast_array.sh to run blastn locally on eukaryote genome assemblies (see [**blastdb_download.sh**](https://github.com/npaulat/bat_ht/blob/main/blastdb_download.sh) script for databases used)
14. Run [**generate_ht_summary_90_90.py**](https://github.com/npaulat/bat_ht/blob/main/generate_ht_90_90_summary.py) to determine species with 20+ 90/90/90 hits (ht_te_list)
    ...
    * Use potential_ht_3_90_sp_hits_summary.csv and ht_te_list as input for [**generate_bash_mamm_cons.py**](https://github.com/npaulat/bat_ht/blob/main/generate_bash_mamm_cons2.py) to generate array jobs of ext_align scripts on mammals
...
17. Run [**concatenate_te_sp_rep_seqs.py**](https://github.com/npaulat/bat_ht/blob/main/concatenate_te_sp_rep_seqs.py) to group species' consensus sequences by element to form a multi line FASTA for each element, and generate files for [**te_reps_muscle_aln.sh**](https://github.com/npaulat/bat_ht/blob/main/te_reps_muscle_aln.sh).
...
20. Filter putative HT elements for highly similar elements to identify and exclude deletion products of elements within the set of putative HT elements by clustering sequences via a) the cross_match utility of [Phrap](http://www.phrap.org/phredphrapconsed.html) v0.990319 with default settings, and b) a modified CD-HIT search with the utility [ClusterPartialMatchingSubs.pl](https://currentprotocols.onlinelibrary.wiley.com/doi/abs/10.1002/cpz1.154) using default settings.
    * Further identification of deletion products by manually comparing aligned sequences.
21. Estimate average age of a TE subfamily in a given species using the average modified Kimura 2-parameter (K2P) distance from the library consensus and the neutral mutation rate of the species. Modified K2P values calculated in [RepeatMasker](http://repeatmasker.org/)'s utilities.
